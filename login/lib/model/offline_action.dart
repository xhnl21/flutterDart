...
class OfflineAction {
  static final OfflineAction _instance = OfflineAction._internal();
  late final Box<Offline> _offlineBox;
  bool _isInitialized = false;

  factory OfflineAction() {
    return _instance;
  }

  OfflineAction._internal();

  Future<void> initialize() async {
    if (_isInitialized) return; // Evita inicializar si ya está hecho

    if (kIsWeb) {
      Hive.init('offline'); // Para la web
    } else {
    ...
    // if (!Hive.isBoxOpen('offline')) { // Verifica si la caja ya está abierta
      _offlineBox = await Hive.openBox<Offline>('offline');
    // }
  ...
  Future<void> insert(List<dynamic> data) async {
    var offline = Hive.box<Offline>('offline');
  ...
  Future<void> clearAllData() async {
    ...
    var communityBox = Hive.box<Offline>('offline');
  ...
  void closeAllBoxes() {
    if (Hive.isBoxOpen('offline')) Hive.box('offline').close();
  }
}